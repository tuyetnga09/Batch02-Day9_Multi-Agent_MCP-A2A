...
class State(TypedDict):
    ...
    privacy_analysis: Annotated[str, _last_wins]  # Bài 4.1: field cho privacy agent
    final_response: str
...
def check_routing(state: State) -> list[Send]:
    ...
    # Bài 4.2: routing cho privacy_agent khi câu hỏi liên quan dữ liệu/quyền riêng tư
...
# Bài 4.1: implement privacy_agent
...
Tập trung: GDPR, data protection, privacy rights, data breach, nghĩa vụ thông báo rò rỉ."""
...
def aggregate_results(state: State) -> dict:
    ...
    # Bài 4.1: thêm privacy_analysis vào báo cáo tổng hợp
    if state.get("privacy_analysis"):
        sections.append(f"🔒 PHÂN TÍCH QUYỀN RIÊNG TƯ / GDPR:\n{state['privacy_analysis']}")
...
def build_graph() -> StateGraph:
    ...
    # Add nodes
    # Lưu ý: check_routing trả về list[Send] nên KHÔNG phải node thường,
    # mà là hàm routing dùng trong add_conditional_edges (giống stage 4).
    graph.add_node("law_agent", law_agent)
    graph.add_node("tax_agent", tax_agent)
    graph.add_node("compliance_agent", compliance_agent)
    graph.add_node("privacy_agent", privacy_agent)  # Bài 4.1
    graph.add_node("aggregate_results", aggregate_results)

    # Define edges
    graph.add_edge(START, "law_agent")
    # law_agent xong -> check_routing quyết định dispatch song song tới agent nào
    ...
    graph.add_edge("privacy_agent", "aggregate_results")  # Bài 4.1
